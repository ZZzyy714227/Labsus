...
## 2026-08-30 — 全链手册：记忆主干与连接地图（合订本交付，学习 0007）

需求：11 讲讲义学完仍记不住细节，需要一份"核心讲义合订版"——清晰脉络、精华设计思想、
模块连接，从最底层物理数学逐步搭建，但不同于分散版（分散的记不住）。

grill-me 八问定稿：定位 = 记忆主干+连接地图（非浓缩讲义）；骨架 = 三层金字塔
（地基→机构→系统）+ 单主线 12 站；卡片 = 五要素（身份/公式/思想/锚点/连接）+面试一击；
一头（定位宣言）一尾（L0-L4 假设清单）；面试视角从四个底层问题出发（不被 100 题库带偏）；
输出 Markdown → PDF（`learning/全链手册.md` + .pdf 10 页）。

内容：第 0 章定位宣言（为什么做/可信度三层+缺口/能力边界//api/v3 数据流）→ 第 1-3 章
三层金字塔模块卡片 → 第 4 章单主线全景（一条链 12 站 + 三层记忆口诀）→ 第 5 章
模型假设 L0-L4（假设→买到→破在哪→升级，面试满分结构）→ 附录 27 个 golden 数字总表。

验证：数字与引擎实测逐位交叉校验（残差 18 条、rc 55.258、scrub 54.578、μ 2.286、
2.18/13.7Hz）；PDF 10 页文本层完整；36 个公式全部 SVG 内联（零渲染失败）。

工具：`learning/tools/md2pdf_html.js`（Markdown→HTML + mathjax-full SVG 服务端渲染 →
headless Edge 打印转 PDF）。记录：`learning/learning-records/0007-全链手册.md`。

## 2026-08-30 — 全链手册渲染修复：text 模式非法下划线 → merror 黑条（学习 0007 补）

用户截图发现"核心公式"标题下是黑色空条——病根：MathJax 对语法错误**输出 merror 黑条
而不抛异常**，旧转换器只 catch 异常导致初检"渲染失败=0"假阴性。实际 2 处公式在
`\text{}` 内写了 `a_y`/`g_y`（下划线在 LaTeX text 模式非法 → `'_' allowed only in math mode`）：
- L279 `\text{与悬架软硬无关！只由 m/a_y/h/t 决定}` → 拆出 `m,a_y,h,t` 到数学模式
- L298 `\text{前轴份额；g_y 变号...→...}` → 拆出 `g_y` + `\rightarrow`

修复两处 + 转换器升级：渲染后检测 `data-mjx-error/merror`，任何公式错误 exit 2 并列出
（杜绝假阴性再次发生）。验证：HTML 中 merror 残留 0、24 个公式块/36 个 SVG 全部成功；
PDF 15 页、绘制对象 52-53/页（公式 SVG 路径进入图形层）、像素级深色比例 0.00-0.15%（无黑条）。
转 PDF 命令需先 `npm install --no-save --prefix %TEMP%/md2pdf mathjax-full@3`。

## 2026-08-30 — G4 讲义勘误波次：十一讲全链审查 + 约 70 处修正（与修复后代码同步）

背景：G1–G3 修复收官后，用户要求"逐一审查十一讲讲义，修改写得不好/过时/草率/简单的
内容"。五路并行子代理逐讲全读 + 引擎源码取证 + 运行时实测，全部修正亲手落稿。

**核心勘误（按严重度）**：
1. **第十讲 ωh<2 稳定边界归因错误**：显式 Euler 对无阻尼振子无条件不稳定（特征值 1±iωh 恒出单位圆），边界属于半隐式/辛积分器（stepDyn 实际所用）——旧讲义把边界归给显式 Euler 且自相矛盾。
2. **第五讲 RC 差异归因勘误**："2.27mm 100% 来自横向偏移"被自算证伪（垂向 −4.93mm 贡献 −2.52mm 主导、横向仅 +0.25mm）。
3. **第三讲转向轴裂缝、第五讲 kg bug、第六讲摇臂三病例、第八讲 μ/D 敏感性**：全部从"当前问题/待修"改为"已修复+测试锁定"（与 R-0830-1/2/3 对齐），含 golden 数值（scrub 54.578/trail 24.643）。
4. **第三/四讲 19 方程口径**：按装配分列（默认 pushrod→lca 18 条 / knuckle 19 条），DOF 审计修正（只给轮跳→秩15→DOF3；传动自由度 2 归 dof() 语义）——子代理实测戳破。
5. **第七讲两层叙事改写**：外层残差不含 pose 重分配（载荷分配是单次映射）——与代码实现对齐，去除"载荷随位形再分配"的过冲描述；实验 3"不对称性"结论被自带数据证伪，改"中位偏移"读法。
6. 其余：steer_axis 旧约定残余、_estimate_rotation 对账签名、Slerp 承诺兑现、grep 全部 11 讲标签平衡验证。

**记录**：`learning/learning-records/0006-lectures-synced.md`（逐讲修正表 + 存量改动复核 + 遗留登记）。
**遗留**：三/四讲的"ℝ¹⁸→ℝ¹⁹"教学基线口径、恶化几何实验脚本未补（登记可选改进）；讲义为离线 HTML，MathJax 依赖 CDN（现有设计）。
