...
- **2026-06-09 比例优化（Approach A: Production FSAE Reference）**: 视觉比例与气动平衡修正,见 `docs/superpowers/specs/2026-06-09-aero-proportions-design.md` 与 `docs/superpowers/plans/2026-06-09-aero-proportions.md`。
  - **车体节点**:`BODY_NOSE_TOP` 160→130,`BODY_ENG_TOP` 380→260,`BODY_ENG_MID_*` 280→200(放低鼻锥+引擎舱,改善尾翼入流)
  - **前翼重做**:`reference_point` [150,0,30]→[500,0,40],`span` 650→1300(>前轮距 outwash),加 1 个襟翼变 2 元素,端板 50/40→25/15,挂点从失效的 `CH2` 改回 `BODY_NOSE_MID_R` / `CH2_L`(连接线恢复可见)
  - **尾翼收小**:`reference_point Z` 550→480,主翼弦长 300→240,襟翼一 280→220,襟翼二 200→150,`z_offset` -75/-140→-50/-95,端板 80/100→30/40
  - **回滚保险**:`src/config.py.bak.2026-06-09` 整体备份 + 4 阶段小步可逆
- **底板+扩散器（待做）**：底部平整面+尾部上翘曲面
- **侧箱（待做）**：前后轮之间车身包覆
