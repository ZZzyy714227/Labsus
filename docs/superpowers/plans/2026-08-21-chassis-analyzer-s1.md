...
**Tech Stack:** Python 3.11+、numpy、scipy（least_squares/interp1d）、pytest。现有依赖链：`src/solver/mechanism/{models,project,solver}.py`（复用，不改签名）、`src/core/models.py`（复用 CaseVersion 载荷输入）。
