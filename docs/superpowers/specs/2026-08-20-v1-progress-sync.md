...
## 八、后续 P4：工程迭代工作流（主体已完成）

> **状态（2026-08-20 更新）**：后端交付 A/B 对比（/api/v2/compare）、敏感性分析
> （/api/v2/sensitivity，硬点 ±1mm → 指标矩阵）、导出（/api/v2/export JSON/CSV）、
> 版本回退与工况复用（复用 store + 对比覆盖）、残差/假设/状态随对比与敏感性输出。
> 见 DEVLOG 2026-08-20 P4 条目。前端展示层按设计文档 §15 冻结不阻塞主链路（P4 后续/独立阶段）。
