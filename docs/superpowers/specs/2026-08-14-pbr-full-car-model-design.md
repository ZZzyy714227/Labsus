...
日期：2026-08-14
状态：**v2 已批准（用户选定路线 A，授权直接实施）**
来源提示词：`docs/FSAE_3D仿真提示词.md`

## 修订记录

- **v1**：复用现有 FRAME_TUBES/BODYWORK_FACES 等几何数据的 PBR 渲染方案
- **v2（2026-08-14）**：用户否决对旧建模数据的依赖（"过去的建模挺烂的"），选定**路线 A：真实尺寸重建**。本规格中所有"复用现有几何数据"的表述作废，替换为"重建"：
  - 几何基线按 `docs/superpowers/specs/2026-08-14-geometry-audit-report.md` §5 基线参数全量重建
  - 重建顺序与任务分解见实施计划 `docs/superpowers/plans/2026-08-14-true-scale-pbr-rebuild.md`
  - 渲染架构（§4.3 模块、§4.5 材质、§4.6 点阵、§4.7 视角）保持 v1 不变，仅数据源变为重建后的新配置
  - 求解器代码（bump/steering/rocker/tire/metrics）属代码而非建模数据，**不动**，只替换其输入参数值
