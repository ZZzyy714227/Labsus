...
const DIMS = [
  { label: '■ 运动学', color: 'teal', keys: ['camber_delta_f', 'camber_delta_r', 'bump_steer_f', 'bump_steer_r', 'caster_delta_f', 'caster_delta_r', 'kpi_delta_f', 'scrub_delta_f'] },
  { label: '■ 姿态侧倾', color: 'purple', keys: ['roll_gradient', 'rc_height', 'anti_dive', 'anti_squat'] },
  { label: '■ 动力学', color: 'silver', keys: ['ride_freq_f', 'ride_freq_r', 'freq_ratio', 'damping_ratio_f', 'damping_ratio_r', 'motion_ratio_f', 'motion_ratio_r', 'load_transfer_f'] },
  { label: '■ 结构受力', color: 'dark', keys: ['pushrod_force', 'uca_lca_force'] },
];

function lightColor(light) {
  return light === 'green' ? get('good') : light === 'yellow' ? get('warn') : get('bad');
}
